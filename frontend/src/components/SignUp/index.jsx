...
const SignupModal = () => {
  ...
  const emailCheckMutation = useMutation(checkEmailDuplicate, {
    onSuccess: (response) => {
  ...
  const sendVerificationMutation = useMutation(sendVerificationCode, {
    onSuccess: () => {
      setVerificationCodeSent(true);
      alert('인증코드가 이메일로 전송되었습니다.');
    },
  ...
  const verifyEmailMutation = useMutation(verifyEmailCode, {
    onSuccess: () => {
      setIsEmailVerified(true);
      alert('이메일 인증에 성공했습니다.');
    },
  ...
  const nicknameCheckMutation = useMutation(checkNicknameDuplicate, {
    onSuccess: (response) => {
